...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        kwargs = kwargs | prompts.meta_info.get("sampling_params", {})
        ...
        with self.update_sampling_params(**kwargs):
            ...
            pad_length = max(self.sampling_params.max_tokens, self.config.response_length)
            response = pad_2d_list_to_length(response, self.pad_token_id, max_length=pad_length).to(
                idx.device
            )
            if self.config.calculate_log_probs:
                rollout_log_probs = pad_2d_list_to_length(
                    rollout_log_probs, -1, max_length=pad_length
                ).to(idx.device)
